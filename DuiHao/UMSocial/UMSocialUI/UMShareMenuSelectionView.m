...
#define UMSocial_Item_Count_PerRow 3 //列数 （计算高度时会用到）
...
#define UMSocial_Menu_CornerRadius 4
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CGFloat UMSocial_Item_Width = 100;
        if ([UIScreen mainScreen].bounds.size.width > [UIScreen mainScreen].bounds.size.height) {
            UMSocial_Item_Width = [UIScreen mainScreen].bounds.size.height/375 * 70;
        }else{
            UMSocial_Item_Width = [UIScreen mainScreen].bounds.size.width/4;
        }
...
- (void)creatCancelButton
{
    ...
    cancelButton.frame = CGRectMake(0, 0, self.frame.size.width, 50);
