...
#define TITLE_COLOR [UIColor colorWithWhite:0 alpha:0.5]
...
-(CGPoint)getCenterPointForIndex:(int) i{
    return CGPointMake((i/(float)(titlesArr.count-1)) * (self.frame.size.width-RIGHT_OFFSET-LEFT_OFFSET) + LEFT_OFFSET, i==0?self.frame.size.height-55:self.frame.size.height-55);
}
...
-(id) initWithFrame:(CGRect) frame Titles:(NSArray *) titles{
    if (self = [super initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, 70)]) {
        ...
        [_handler setFrame:CGRectMake(LEFT_OFFSET, 8, 25, 25)];
