...
- (void)upDataScore {
    ...
    OnceLogin *onceLogin = [OnceLogin getOnlyLogin];
    if (self.examModel.examId.length) {
        NSDictionary *dic = @{EXAMSETID: self.examModel.examId, SCHOOLNUMBER: onceLogin.schoolNumber, TEACHERALIASNAME: self.course.teacherAliasName, COURSEALIAS: self.course.courseAlias, STUDENTID: onceLogin.studentID, SCORE: [NSString stringWithFormat:@"%d", [self getScore]], WRONG: self.wrongDataSource};
