...
@property (nonatomic, copy  ) NSArray        *beginSource;
@end
...
- (instancetype)initWithDataSource:(NSArray *)dataSource
    ...
    if (self) {
        ...
        self.beginSource = ((NSArray*)self.dataSource[2]);
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (component == 1) {
        NSMutableArray *mutableArr = [NSMutableArray arrayWithArray:self.beginSource];
        for (int i = 0; i < row; i++) {
            [mutableArr removeObjectAtIndex:0];
        }
//        NSArray *array = (NSArray*)[self.dataSource lastObject];
//        array = mutableArr;
        self.dataSource = @[self.dataSource.firstObject, self.dataSource[1], mutableArr];
